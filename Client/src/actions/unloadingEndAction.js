...
export const updateUnloadingEnd = (data) => async (dispatch) => {
    ...
    try {
        const response = await axios.put('https://maha-lrki.onrender.com/api/v3/unloadingEnd/unloadingEndAddUpdate', data);
...
export const fetchUnloadingEnd = () => async dispatch => {
    dispatch(fetchUnloadingRequest());
    try {
      const response = await axios.get('https://maha-lrki.onrender.com/api/v3/unloadingEnd/get-unloadingEndTps');
