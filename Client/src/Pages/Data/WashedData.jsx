...
const WashedData = React.memo(() => {
  ...
  const washedCoalEntries = entries["Washed Coal"];

  if (!washedCoalEntries || washedCoalEntries.length === 0) {
    return <div>No washed coal entries available.</div>;
  }

  let filteredEntries = [...washedCoalEntries];
          ...
          <tbody>
            {filteredEntries.map(entry => (
              <tr key={entry._id}>
                ...
                <td>{entry.selectWasheryOperator}</td>
...
WashedData.displayName = 'Washed Coal';

export default WashedData;
